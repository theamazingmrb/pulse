...
import { Journal, Checkin } from "@/types";
...
export default function HomePage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold">Priority Compass</h1>
  ...
  // Reshape journal_tasks into tasks array
              ...
              <ul className="flex flex-col gap-2">
                {tasks.map((t) => (
                  <li key={t.id} className="flex items-center gap-2 text-sm">
                    ...
                    {t.project && (
                      <span className="ml-auto text-xs text-muted-foreground flex-shrink-0">{t.project}</span>
                    )}
                  </li>
                ))}
              </ul>
